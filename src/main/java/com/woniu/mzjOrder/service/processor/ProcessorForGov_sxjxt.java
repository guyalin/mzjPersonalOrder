...
public class ProcessorForGov_sxjxt implements DocumentProcessor {
    @Override
    public List<ArticleRecord> findAndExplainToArticleRecord(Document document, UrlMonitorEntity urlMonitorEntity) {
        ...
        Element e = document.select("div.main_secRight").first();
        Elements es = e.select("li");
        TitleRule titleRule = new TitleRule("a[href]",0,false,true,"title");
        DateRule dateRule = new DateRule("span.spanT",0, TextLocationEnum.OWNER ,"");
