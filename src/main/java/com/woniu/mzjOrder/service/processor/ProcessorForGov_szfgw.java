...
public class ProcessorForGov_szfgw implements DocumentProcessor {
    @Override
    public List<ArticleRecord> findAndExplain(Document document, UrlMonitorEntity urlMonitorEntity) {
        ...
        Element e = document.select("div.con").first();
        Elements es = e.select("li");
        TitleRule titleRule = new TitleRule("span",0,true,false,"");
        DateRule dateRule = new DateRule("span",1, TextLocationEnum.OWNER, "");
